...
import com.ultraflash.equi3ae.grid.AEPartGridBlock;
...
import com.ultraflash.equi3ae.reference.TextureManager;
 ...
 */
public abstract class PartBaseE3AE implements IPart, IGridHost, IActionHost, IPowerChannelState {
   ...
   private final AEPartGridBlock gridBlock;
    ...
    public PartBaseE3AE( final AEPartEnum associatedPart, final SecurityPermissions ... interactionPermissions ) {
    ...
    public void addToWorld()
    {
        ...
        if( ( this.hostTile != null ) && ( this.host != null ) && ( this.hostTile.getWorldObj() != null ) )
        {
            ...
            {
                LogHelper.error( e+ " Part (%s) was unable to update it's node. The part may not function correctly "+
    ...
    public final boolean isPartUseableByPlayer( final EntityPlayer player )
    {
        ...
        if( !this.canPlayerInteractWith( player, this.hostTile ) )
        {
            return false;
        }
    ...
    public boolean onActivate( final EntityPlayer player, final Vec3 position )
    {
        ...
        {
            // Launch the gui
            GuiHandlerE3AE.launchGui( this, player, this.hostTile.getWorldObj(), this.hostTile.xCoord, this.hostTile.yCoord, this.hostTile.zCoord );
        }
    ...
    {
        // Read the owner
        if( data.hasKey( PartBaseE3AE.NBT_KEY_OWNER ) )
        {
            this.ownerID = data.getInteger( PartBaseE3AE.NBT_KEY_OWNER );
        }
    }
    ...
    public void writeToNBT( final NBTTagCompound data, final PartItemStack saveType )
    {
        ...
        {
            // Set the owner ID
            data.setInteger( PartBaseE3AE.NBT_KEY_OWNER, this.ownerID );
        }
